...
public sealed class ScanController
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var effectiveOcrEngine = ResolveOcrEngine(options);
        var duplicateGuard = new DuplicateGuard(Math.Max(1, profile.DuplicateRowThreshold));

        scanLog.Write($"Start scan. Profile={profile.Name}, Traversal={traversalMode}, Process={options.ProcessName}, MaxItems={options.MaxItems}, Rarities={string.Join(",", options.Rarities)}, BringToFront={options.BringToFront}, StopAtNonLevel15={options.StopAtNonLevel15}, OcrEngine={options.OcrEngine}, EffectiveOcrEngine={effectiveOcrEngine}, HighSpeedOcr={options.HighSpeedOcr}, OcrWorkers={ocrWorkerCount}, OcrBatchSize={options.OcrBatchSize}, OcrQueueCapacity={queueCapacity}, OcrIntraOpThreads={ocrIntraOpThreads}, OcrSampleLimit={options.OcrSampleLimit}");
        if (options.OcrEngine == OcrEngineMode.ZzzFastFieldAware && effectiveOcrEngine != OcrEngineMode.ZzzFastFieldAware)
        {
            scanLog.Write($"ZZZ fast OCR runtime is not verified yet; falling back to PP-OCRv5. Expected future assets: Model={AppPaths.ZzzFastModelFile}, Dict={AppPaths.ZzzFastCharacterDictFile}");
        }
        using var ocrDiagnostics = new OcrDiagnosticsWriter(Path.Combine(outputDir, "ocr_diagnostics.csv"));
        var sampleCollector = new OcrSampleCollector(outputDir, options.OcrSampleLimit);
    ...
    private static OcrEngineMode ResolveOcrEngine(ScanOptions options)
    {
        // The ZZZ-specific model needs a separate verified runtime because its planned
        // YAS-style tensor shape differs from the current PP-OCRv5 recognizer.
        return OcrEngineMode.PpOcrV5General;
    }
    ...
    private Task[] StartOcrWorkers(
        ...
        OcrSampleCollector sampleCollector)
    {
        return Enumerable.Range(1, workerCount)
            .Select(workerId => Task.Run(() => ConsumeOcrWorker(queue, ocrResults, outputDir, options, scanLog, workerId, ocrIntraOpThreads, counters, diagnostics, sampleCollector)))
            .ToArray();
    }
        ...
        while (TryTakeBatch(queue, options.OcrBatchSize, out var batch))
        {
            ...
            try
            {
                ...
                for (var i = 0; i < batch.Count; i++)
                {
                    ...
                    mats[i] = mat;
                    sampleCollector.TrySave(capture.Index, capture.Rarity, mat, capture.Rois, scanLog.Write);
                }
                bitmapToMatSw.Stop();
