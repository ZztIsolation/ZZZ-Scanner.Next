...
Scanner 1.0.41 会直接评估捕获画面，而不是读取或猜测 Windows 的 HDR、Auto HDR、
夜间模式或显卡设置。是否打开仓库只由“驱动仓库”标题语义和灰度网格/面板结构共同
确认，不再使用青色拆解按钮或 S/A/B 品质颜色。仓库数量还必须在独立画面中取得两次
一致结果，之后才允许点击或滚动；扫描期间也会用归一化边缘基线持续确认仓库没有退出。
颜色只用于确认后的品质识别、捕获分类和 OCR 路由。反色、极端 LUT、强烈游戏滤镜和
UI 模组仍会安全停止，但不会再因为普通 HDR 或夜间模式误报“未打开仓库”。
...
| 品质 | 官方扫描固定为 S 级 | 已有 A/B 数据继续由计算器仓库保留 |
...
Scanner 1.0.42 正式支持 S 级任意等级和 0-4 条连续副词条。关闭“仅 15 级”后，
合法低等级 S 盘会正常识别并继续扫描；半对缺失或中间断层仍会等待稳定后保守报错。
...
- Fast OCR 只覆盖有限的本地/云端分辨率。回退到 PP-OCR 不代表未知布局一定
  安全。
- 显示色彩兼容只覆盖常见强度的 HDR、夜间模式和显卡色彩调整。扫描器不会提供
  绕过视觉安全门禁的强制继续开关。
...
本仓库负责 Scanner 与 Helper 的源码、版本标签、Windows 构建和 Release 资产；
计算器仓库负责浏览器集成并消费已发布 manifest。Calculator 仓库中既有的 Release
镜像继续用于兼容当前线上版本，未来 manifest 的 GitHub fallback 由本仓库发布。
...
- Scanner 1.0.43 会把 ROI 完整度、最后接受门槛、稳定帧、重试次数、窗口尺寸、DPI、捕获后端和视觉配置作为结构化诊断交给本机网页。计算器可以按其独立隐私说明上传这组脱敏摘要，但不会上传驱动盘内容、OCR 文本、截图、本机路径、完整日志或异常堆栈；用户可在计算器设置中关闭。
