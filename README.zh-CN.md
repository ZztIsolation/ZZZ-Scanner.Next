...
Scanner 1.0.40 会直接评估捕获画面，而不是读取或猜测 Windows 的 HDR、Auto HDR、
夜间模式或显卡设置。常见的高光裁切、温和色温、饱和度、亮度和对比度变化会自动
使用区域色相、相对亮度和图像结构识别；检测到非中性色彩环境时自动使用 PP-OCR。
反色、黑白、极端 LUT、强烈游戏滤镜和 UI 模组仍会安全停止。
这些改动减少了对固定 RGB 的依赖，但不保证兼容所有 HDR、滤镜、驱动增强或色彩管理配置。
